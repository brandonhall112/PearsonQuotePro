...
def _candidate_roots() -> list[Path]:
    ...
    # Common places when running packaged onefile/onedir or dev checkout.
    roots.extend([
        cwd,
        anchor,
        anchor.parent,
        anchor.parent.parent if anchor.parent != anchor else anchor,
    ])

    for base in (cwd, anchor, anchor.parent):
        for name in ("CommissionPro", "PearsonCommissioningPro", "PearsonCommissionPro"):
            roots.append((base / name).resolve())
